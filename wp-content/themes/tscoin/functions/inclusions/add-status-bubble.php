...
function true_status_custom()
{
	register_post_status('archive', array(
		'label' => 'Затвердженно',
		'label_count' => _n_noop('Затвердженно <span class="count">(%s)</span>', 'Затвердженно <span class="count">(%s)</span>'),
		'public' => true,
...
function true_append_post_status_list()
{
	...
	$optionselected = '';
	$statusname = '';
	if ($post->post_type == 'result') { // если хотите, можете указать тип поста, для которого регистрируем статус, а можете и вовсе избавиться от этого условия
		if ($post->post_status == 'archive') { // если посту присвоен статус архива
			$optionselected = ' selected="selected"';
			$statusname = "$('#post-status-display').text('Затвердженно');";
		}
		/*
		...
		echo "<script>
		jQuery(function($){
			$('select#post_status').append('<option value=\"archive\"$optionselected>Затвердженно</option>');
			$statusname
		});
		</script>";
	}
...
function add_status()
{
	...
	jQuery(document).ready( function($) { 
		$( 'select[name=\"_status\"]' ).append( '<option value=\"archive\">Затвердженно</option>' );
	});
	</script>";
}
...
{
	global $menu;
	$count1 = wp_count_posts('orders')->pending;
	if ($count1) {
